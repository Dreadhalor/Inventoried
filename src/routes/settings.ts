...
router.post('/set_settings', (req, res) => {
  ...
  if (settings){
    ...
    let promises = [];
    if (durablesCategories) promises.push(dbClient.setDurablesCategories(durablesCategories));
    if (consumablesCategories) promises.push(dbClient.setConsumablesCategories(consumablesCategories));
    if (manufacturers) promises.push(dbClient.setManufacturers(manufacturers));
    if (tags) promises.push(dbClient.setTags(tags));
