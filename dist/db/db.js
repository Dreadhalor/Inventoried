...
const save2 = exports.save2 = (info) => {
    let save = formatIfElseDuplicate(info, formatUpdateValuesPrepareString2(info), formatInsertValuesPrepareString3(info));
    let tableCreate = formatCreateTableIfNotExists(info.tableName, info.columns.map(column => column.name), info.columns.map(column => column.dataType));
...
const preparedStatementWithInputs3 = (ps, info) => {
    if (!ps)
